...
export default async function OperationsRequestsPage({
  ...
  const requests = await getOperationsRequests(filters);
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-slate-600">
        Rental reservations and purchase inquiries for Wander Bikes only.
      </p>
      <form className="mt-7 flex flex-wrap gap-3 rounded-[0.9rem] border border-slate-200 bg-white p-4">
        <select
          name="status"
          defaultValue={filters.status ?? "all"}
          className="market-select mt-0 w-full sm:w-48"
        ...
        {hasFilters ? (
          <Link href="/operations/requests" className="btn-quiet text-sm">
      ...
      {requests.length > 0 ? (
        ...
        <div className="mt-4 rounded-[0.9rem] border border-dashed border-slate-300 bg-white px-6 py-12 text-center text-sm text-slate-600">
          No Wander requests match these filters.
        </div>
      )}
    </div>
  );
