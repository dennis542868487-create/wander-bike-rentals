...
const items = [
  ...
  { href: "/admin/listings", label: "Listings", icon: Bike },
  { href: "/admin/safety", label: "Safety", icon: ShieldAlert },
  { href: "/admin/requests", label: "Requests", icon: MessagesSquare },
    ...
    <div className="min-h-screen bg-[#f5f7f9]">
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white">
        <div className="flex h-16 items-center justify-between gap-4 px-4 sm:px-6 lg:pl-[18.5rem] lg:pr-8">
          ...
          <div className="flex items-center gap-2">
            <Link
              href="/operations"
              aria-label={WANDER_DASHBOARD_LABEL}
              className="btn-secondary min-h-10 px-3 py-2 text-sm"
            >
              <Bike className="h-4 w-4" aria-hidden="true" />
              <span className="hidden xl:inline">{WANDER_DASHBOARD_LABEL}</span>
            </Link>
            <Link
              href="/account"
      ...
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-[17rem] bg-slate-950 text-white lg:block">
        <div className="flex h-full flex-col p-4">
          <Link href="/admin" className="px-3 py-3">
      ...
      {drawerOpen ? (
        <div className="fixed inset-0 z-50 lg:hidden" role="presentation">
          <button
            ...
            aria-label={`Close ${PLATFORM_DASHBOARD_LABEL} navigation`}
          />
          <aside
            role="dialog"
            aria-modal="true"
            aria-label={`${PLATFORM_DASHBOARD_LABEL} navigation`}
